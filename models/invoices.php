...
class invoices {
    ...
    function create($ref, $company, $due_date){
        ...
        $stmt->bindParam(':due_date', date("Y/m/d", strtotime($due_date)), PDO::PARAM_STR);
        $stmt->execute();
    ...
    function update($id, $ref, $company, $due_date){
        ...
        $stmt = $connect->prepare("UPDATE invoices SET ref = :ref, company_id = (SELECT id FROM companies WHERE name LIKE :company), updated_at = :updated_at, due_date = :due_date WHERE id = :id");
        $stmt->bindParam(':ref', $ref, PDO::PARAM_STR);
        $stmt->bindParam(':company', $company, PDO::PARAM_STR);
        $stmt->bindParam(':updated_at', date('Y/m/d H:i:s'), PDO::PARAM_STR);
        $stmt->bindParam(':due_date', date("Y/m/d", strtotime($due_date)), PDO::PARAM_STR);
